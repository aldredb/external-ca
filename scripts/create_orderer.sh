...
openssl req -config openssl_root-orderer_tls.cnf -new -x509 -sha256 -extensions \
v3_ca -key $tlsRCA/private/rca.tls.orderer.example.com.key \
...
openssl ca -batch -config openssl_root-orderer_tls.cnf -extensions v3_intermediate_ca \
